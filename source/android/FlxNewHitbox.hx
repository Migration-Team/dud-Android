...
class FlxNewHitbox extends FlxSpriteGroup
{
	...
	public function new():Void
	{
		...
		if (ClientPrefs.sbHitbox)
		{
			add(buttonLeft = createHint((FlxG.width / 2) + (FlxG.width / 4), 0, Std.int(FlxG.width / 4), FlxG.height, 0xFF00FF));
			add(buttonDown = createHint(FlxG.width / 2, 0, Std.int(FlxG.width / 4), FlxG.height, 0x00FFFF));
			add(buttonUp = createHint(FlxG.width / 4, 0, Std.int(FlxG.width / 4), FlxG.height, 0x00FF00));
			add(buttonRight = createHint(0, 0, Std.int(FlxG.width / 4), FlxG.height, 0xFF0000));
		} else {
